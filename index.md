---
layout: my_default 
title:  " "
permalink:
---

###  <marquee> 🚧 My site is currently under construction, thanks for stopping by! 🚧  <marquee>

## About Me

I'm a 5th year PhD student in [Macromolecular Science and Engineering](https://macro.engin.umich.edu/) and [Scientific Computing](https://micde.umich.edu/ph-d-in-scientific-computing/) at the University of Michigan.

I'm also a committer for the data management framework [signac](https://signac.io).

You can find my [CV here](atravitz_CV_2020.pdf).

<br>

<br>

## Research
### [on-the-fly autocorrelation with HOOMD-blue](https://github.com/atravitz/correlator-hoomd-plugin)
A HOOMD-blue plug-in that allows for on-the-fly multi-tau autocorrelation, using the code accompanying [this paper].(https://aip.scitation.org/doi/10.1063/1.3491098)


<br>
### [an MBAR python package](https://github.com/atravitz/mbar_pmf)
My first python package, a helper for wrangling my simluation data for PMF calculations. Submitted as my final project for ChE 696 with Prof. Heather Mayes.

<br>

<br>

## Outreach

### [REACT](https://www.reactmi.org/)

In 2017 I co-founded REACT, a free one-day workshop for K-12 teachers led by grad students at U of M.
Check out the website for [downloadable activity guides](https://www.reactmi.org/for-teachers/classroom-activities)!


<br>

<br>

## Other Fun Things

### [modeling the CTA with networkx](https://github.com/atravitz/transit_models)
My final project for CMPLXSYS 535 (Computer Modeling of Complex Systems) with Prof. Marisa Eisenberg.

